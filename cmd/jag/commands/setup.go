...
func SetupCmd(info Info) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if printPath != "" {
				return fmt.Errorf("invalid value for --print-path: %s", printPath)
			}
	...
	cmd.Flags().BoolP("skip-assets", "s", false, "if set, will skip the assets download")
	...
	cmd.Flags().Bool("keep-old", false, "keep old SDK versions (persistent: 'jag config cache set-keep-old')")
	return cmd
}
